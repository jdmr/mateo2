	...
	<body>
		...
		<div id="show-tipoProducto" class="content scaffold-show" role="main">
			...
			<ol class="property-list tipoProducto">
				...
				<g:if test="${tipoProductoInstance?.almacen}">
				<li class="fieldcontain">
					<span id="almacen-label" class="property-label"><g:message code="tipoProducto.almacen.label" default="Almacen" /></span>
					
						<span class="property-value" aria-labelledby="almacen-label"><g:link controller="almacen" action="show" id="${tipoProductoInstance?.almacen?.id}">${tipoProductoInstance?.almacen?.encodeAsHTML()}</g:link></span>
